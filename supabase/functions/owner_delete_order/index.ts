...
serve(async (req) => {
    ...
    try {
        ...
        if (order.org_id !== profile.org_id) {
            return new Response(
                JSON.stringify({ error: 'Unauthorized to delete this order' }),
                { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }

        // Delete related records first (order_items, order_events, notifications)
        await supabaseAdmin.from('order_events').delete().eq('order_id', order_id)
