    ...
    <div className="w-full max-w-[1400px] mx-auto px-6 py-24 space-y-16">
      ...
      <div className="flex overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4 gap-6 no-scrollbar snap-x snap-mandatory pb-4">
        {displayCategories.map((cat) => {
          ...
          // Use metadata from DB if available, otherwise fallback to constants
          const meta = {
            image: cat.image || CATEGORY_METADATA[cat.id]?.image || CATEGORY_METADATA['mains']?.image,
            vibe: cat.vibe || CATEGORY_METADATA[cat.id]?.vibe || CATEGORY_METADATA['mains']?.vibe
          };
